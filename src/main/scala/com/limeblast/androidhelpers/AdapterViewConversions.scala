...
sealed trait HasItemLongClickListener extends ImplicitFunctionToItemLongListener {
  def self: AdapterView[_]

  def onItemLongClick(f: (AdapterView[_],View, Int, Long) => Boolean): Unit = self.setOnItemLongClickListener(f)
}

trait AdapterViewConversions {
  implicit def adapterViewHasItemLongClickListener(adapter: AdapterView[_]): HasItemLongClickListener = new HasItemLongClickListener {
    val self = adapter
  }
}
